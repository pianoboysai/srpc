...
from gevent import spawn, spawn_later,kill, spawn_link_exception
...
class SocketRPCProtocol:
    ...
    def connection_made(self):
        #logging.info('New connection from %s:%s' % self.address)
        if self.factory:
            self.factory.connection_made()

    def connection_lost(self,managed=False):
        #logging.info('Lost connection from %s:%s' % self.address)
        if managed:
            self.closing = True
            
        if self.read_greenlet:
            kill(self.read_greenlet)
        if self.write_greenlet:
            kill(self.write_greenlet)
    ...
    def dispatch_call(self, method, id, args, kwargs):
        ...
        try:
            ...
            print 'Unknown CALL method %s (%d)' % (method, id)
...
class SocketRPCServer(StreamServer):
    ...
    def handle(self, socket, address):
        ...
        protocol.write_greenlet = spawn(protocol.handle_write)
...
class SocketRPCClient(object):
    ...
    def connect(self):
        ...
        try:
            ...
            self.protocol.read_greenlet =  spawn_link_exception(self.protocol.handle_read)
            self.protocol.write_greenlet = spawn_link_exception(self.protocol.handle_write)
    ...
    def connection_lost(self):
        #logging.info('Lost connection to %s' % self.address)
    ...
    def close(self):
        self.protocol.connection_lost(True)
